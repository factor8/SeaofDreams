...
//Custom Constructor
Sparkle::Sparkle(uint32_t color, uint8_t density, uint8_t del){
  int total=strip.numPixels();
  int r; 
  int i;
  int pixel; 
  ...
  for( r=0; r < density; r++ )  {
   pixel = TrueRandom.random(1,total);
strip.setPixelColor(pixe;, color);
  }
...
void Sparkle::SpeedChange(uint8_t from, uint8_t to, uint8_t duration, uint8_t wait){
 int go;
 for(go=0;go<100;go++) {
   SinglePixel( Easing::easeInCubic(go, from, to, duration) );
 }
 delay(wait)
}

void SparkleSlowFast(){
	Sparkle speedup = Sparkle();
}

void SparkleFastSlow(){
	
}

void Sparkle::Faster(uint8_t from, uint8_t to, uint8_t duration, uint8_t wait){
 int go;
 for(go=0;go<100;go++) {
   SinglePixel( Easing::easeInCubic(go, from, to, duration) );
 }
...
void Sparkle::Slower(uint8_t from, uint8_t to, uint8_t duration, uint8_t wait){
 int go;
 for(go=0;go<100;go++) {
   SinglePixel( Easing::easeInCubic(go, from, to, duration) );
 }
...
void SinglePixel(){
 Sparkle singlepixel = new Sparkle(rgba(255,255,255,1), 1, 20);
}

void SinglePixel(uint32_t color, uint8_t wait){
 Sparkle singlepixel = new Sparkle(color, 1, wait);
}
